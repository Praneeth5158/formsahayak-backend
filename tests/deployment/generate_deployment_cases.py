...
CAT_INFO = {
    ...
    "scenarios": [('Verify Dockerfile builds FastAPI image successfully', 'Dockerfile', 'FastAPI app container built with exit code 0', 'High', 'Critical', 'CI-CD Pipeline Tests'), ('Verify Render deployment succeeds from repository trigger', 'GitHub integration', 'Render container triggers build and finishes success status', 'High', 'Critical', 'Render Deployment'), ('Verify database migration run successfully on deploy', 'alembic upgrade head', 'Schema updated to latest version in production', 'High', 'Critical', 'Database Migrations'), ('Verify server startup checks validate DB connection', 'DATABASE_URL environment', 'App checks DB status and logs successful connection', 'High', 'Major', 'Environment Configurations'), ('Verify database migration rollback reverts changes safely', 'alembic downgrade -1', 'Database schema rolled back to previous state', 'High', 'Critical', 'Database Migrations'), ('Verify dynamic scaling scales instances based on CPU threshold', 'CPU load > 80%', 'Render provisions additional backend instances', 'Medium', 'Major', 'Render Deployment'), ('Verify Docker multi-stage build size is optimized', 'builder stage', 'Final production image size is under 300MB', 'Low', 'Minor', 'CI-CD Pipeline Tests'), ('Verify missing environment variables halt startup', 'unset JWT_SECRET', 'App logs missing config error and exits with code 1', 'High', 'Critical', 'Environment Configurations'), ('Verify SSL certificate is automatically applied on Render URL', 'HTTPS request', 'SSL certificate verified, connection is TLS 1.3', 'High', 'Major', 'DNS & SSL Setup'), ('Verify port binding binds backend to 0.0.0.0:8000', 'PORT env variable', 'FastAPI listens on specified port globally', 'Medium', 'Major', 'Port Binding & Listening'), ('Verify health check endpoint returns 200 during blue-green deployment', 'GET /health', 'Old container serves requests until new one is fully UP', 'High', 'Critical', 'CI-CD Pipeline Tests'), ('Verify MySQL connection pool size limit is set correctly', 'max_overflow=10', 'DB pool scales under load without dropping connections', 'Medium', 'Major', 'Render MySQL Config'), ('Verify static files are served correctly from Render persistent disk', 'GET /static/welcome.png', 'Static asset loaded with HTTP 200 and correct content-type', 'Medium', 'Minor', 'Render Deployment'), ('Verify build fails if tests do not pass in GitHub Actions', 'pytest execution', 'Pipeline aborts deployment step and marks build failed', 'High', 'Major', 'CI-CD Pipeline Tests'), ('Verify timezone setting is configured to IST in production container', 'datetime.now()', 'Timestamp outputs correct time matching timezone setting', 'Low', 'Minor', 'Environment Configurations')]
}

def generate_cases():
    print("Generating Deployment Testing cases...")
    ...
    output_filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), "deployment_test_cases.xlsx")
