...
CAT_INFO = {
    ...
    "scenarios": [('Verify Retrofit successfully serializes signup payload to FastAPI', 'JSON body payload', 'FastAPI parses signup schema without ValidationError', 'High', 'Critical', 'Mobile-API Integration'), ('Verify FastAPI database session queries data from MySQL', 'SQLAlchemy select query', 'Records retrieved and database connection returned to pool', 'High', 'Critical', 'API-Database Integration'), ('Verify FastAPI forwards form image to EasyOCR and receives coordinates', 'Uploaded image file', 'EasyOCR extracts text segments and boundary boxes arrays', 'High', 'Major', 'OCR Service Integration'), ('Verify Groq API client receives formatted prompt and outputs JSON steps', 'LLM prompt builder data', 'Groq returns structured JSON list containing step-by-step guidance', 'High', 'Major', 'Groq-LLM Integration'), ('Verify Room local cache syncs database items to MySQL server', 'Offline inserts list', 'Sync call uploads metadata and matches remote record IDs', 'High', 'Major', 'Mobile-API Integration'), ('Verify FastAPI routes pipe gTTS audio output stream to Retrofit client', 'TTS request text', 'MP3 byte array streamed with header audio/mpeg', 'Medium', 'Minor', 'Mobile-API Integration'), ('Verify session cache gets invalidated when user clicks logout', 'JWT token string', 'Redis token storage updates status to blacklisted', 'Medium', 'Major', 'API-Database Integration'), ('Verify SMTP email service delivers password reset code successfully', 'Reset verification details', 'SMTP email successfully queued and delivered to inbox', 'Medium', 'Minor', 'API-Database Integration'), ('Verify SQLAlchemy handles MySQL lock timeouts gracefully with retries', 'Concurrent write updates', 'Query retries transaction and completes successfully without crashing', 'High', 'Major', 'API-Database Integration'), ('Verify Retrofit AuthInterceptor appends JWT token to all secured routes', 'Auth headers context', 'Header contains Authorization: Bearer <token> on request', 'High', 'Critical', 'Mobile-API Integration'), ('Verify EasyOCR handles blurred image inputs without crashing backend', 'Out of focus document', 'Backend returns error object, files closed cleanly', 'Medium', 'Minor', 'OCR Service Integration'), ('Verify Groq API failures fallback to local rule-based guidance generator', 'Groq timeout (504)', 'System falls back to predefined form schema templates', 'High', 'Major', 'Groq-LLM Integration'), ('Verify database rollback is executed on FastAPI exception', 'Faulty transaction run', 'SQLAlchemy rolls back transaction, MySQL integrity maintained', 'High', 'Critical', 'API-Database Integration'), ('Verify HTTP error responses are mapped to Android API exceptions', 'HTTP 401 Unauthorized', 'Retrofit parses error body and updates VM login state', 'Medium', 'Minor', 'Mobile-API Integration'), ('Verify concurrent file uploads do not exhaust FastAPI worker threads', '10 file upload requests', 'FastAPI utilizes Uvicorn pool workers to queue uploads', 'Medium', 'Major', 'Mobile-API Integration')]
}

def generate_cases():
    print("Generating Integration Testing cases...")
    ...
    output_filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), "integration_test_cases.xlsx")
