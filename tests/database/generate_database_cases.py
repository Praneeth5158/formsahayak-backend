...
CAT_INFO = {
    ...
    "scenarios": [('Verify unique index on user email rejects duplicate registrations', 'Email duplicate registers', 'MySQL throws IntegrityError, transaction rolls back', 'High', 'Critical', 'Schema Constraints'), ('Verify database cascades deletions of user profile tables records', 'Delete user ID = 10', 'Forms, history, and feedback records for user 10 are deleted', 'High', 'Major', 'Schema Constraints'), ('Verify database transaction rollback on intermediate query failure', 'Broken transaction steps', 'First query changes are reverted, database state unchanged', 'High', 'Critical', 'Transaction Rollbacks'), ('Verify foreign key constraints prevent inserting invalid form references', 'Invalid form_id metadata', 'MySQL blocks insert statement and returns foreign key error', 'Medium', 'Major', 'Schema Constraints'), ('Verify indexes exist on frequently searched columns to speed up queries', 'Explain select query', 'Query runs Index Scan instead of full table scan', 'Medium', 'Minor', 'Indexing & Query Performance'), ('Verify connection pool recovers after database connection drops', 'Restart MySQL container', 'SQLAlchemy verifies active links and creates new connections', 'High', 'Critical', 'MySQL Lock Contention'), ('Verify database locks row records during simultaneous updates', 'Simultaneous updates test', 'First request locks row, second waits, deadlock avoided', 'High', 'Major', 'MySQL Lock Contention'), ('Verify query performance scales linearly up to 10,000 user rows', 'Query database logs', 'Select queries return responses under target SLA of 100ms', 'Medium', 'Minor', 'Indexing & Query Performance'), ('Verify schema migrations execute successfully without dropping active data', 'alembic upgrades run', 'Existing tables schema updated, column data is preserved', 'High', 'Critical', 'Schema Constraints'), ('Verify SQL injection attempts are blocked by parameterized queries', 'Email = "\' OR 1=1 --"', 'SQLAlchemy parameterized query treats input as literal string', 'High', 'Critical', 'Schema Constraints'), ('Verify database connection pool limits connection count to specified bounds', '20 concurrency connections', 'Additional queries queue until active sessions release slots', 'Medium', 'Major', 'MySQL Lock Contention'), ('Verify transaction isolation level is set to Repeatable Read by default', 'Dirty read query test', 'MySQL prevents dirty reads across concurrent active sessions', 'Medium', 'Major', 'Transaction Rollbacks'), ('Verify text columns handle multi-byte Unicode characters (utf8mb4)', "Telugu inputs text 'ఫారమ్'", 'Characters stored and retrieved without corruption encoding', 'High', 'Major', 'Schema Constraints'), ('Verify history logs table has partitions set up for scale performance', 'Partition query logs', 'Queries isolate partition records, query speed is maintained', 'Low', 'Minor', 'Indexing & Query Performance'), ('Verify database backup file restores tables and metadata schemas successfully', 'MySQL dump restore', 'Restored tables contain identical row counts and schema bounds', 'Low', 'Minor', 'Schema Constraints')]
}

def generate_cases():
    print("Generating Database Testing cases...")
    ...
    output_filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), "database_test_cases.xlsx")
