...
CAT_INFO = {
    ...
    "scenarios": [('Verify first-time user onboarding screens display tutorial clearly', 'Welcome swipe actions', 'User understands the tool features, onboarding completes successfully', 'High', 'Major', 'First-time User Flow'), ('Verify OCR bounding boxes align correctly on mobile screens', 'Various form images', 'Highlights overlay exact physical coordinates of text boxes', 'High', 'Major', 'Form Guidance Clarity'), ('Verify speech assistant reads instructions at a clear, adjustable pace', 'Speaker pace slider', 'Speech speed speeds up or slows down per user selection', 'Medium', 'Minor', 'Voice Helper Usability'), ('Verify field errors are highlighted on canvas with easy-to-read text warnings', 'Validation error inputs', 'UI draws red highlight outline and reads correction instructions', 'High', 'Major', 'Error Guidance Feedback'), ('Verify user can switch interface to Hindi and follow instructions', 'Hindi language select', 'All buttons, instructions, and vocal directions translate to Hindi', 'High', 'Major', 'First-time User Flow'), ('Verify profile edit fields can be updated easily by non-technical users', 'Edit name and save', 'User updates details without errors, success toast displays', 'Medium', 'Minor', 'First-time User Flow'), ('Verify form history list screen displays submission dates clearly', 'History screen scroll', 'User reviews past forms list with submission status flags', 'Medium', 'Minor', 'Form Guidance Clarity'), ('Verify screen reader accessibility reads OCR highlights', 'TalkBack enabled', 'TalkBack announces highlighted form fields and instructions', 'High', 'Major', 'Voice Helper Usability'), ('Verify feedback screen allows entering comments with emojis', '5-star rating, emoji text', 'Feedback saved, rating submitted, UI returns to home view', 'Low', 'Minor', 'Error Guidance Feedback'), ('Verify dark mode transition displays high contrast text for readability', 'Toggle dark mode', 'UI colors switch, text maintains contrast requirements', 'Medium', 'Minor', 'First-time User Flow'), ('Verify app resumes guidance progress after incoming phone call', 'Call interruption', 'App pauses player, resumes instruction state once call ends', 'Medium', 'Major', 'First-time User Flow'), ('Verify dynamic font scaling matches system font configurations', 'System font scale = Huge', 'Text blocks resize without clipping or overflowing layout bounds', 'Medium', 'Minor', 'Form Guidance Clarity'), ('Verify user can easily copy field values extracted by EasyOCR', 'Long-press text field', 'Text copied to clipboard, success feedback message shows', 'Low', 'Minor', 'Form Guidance Clarity'), ('Verify voice guidance remains audible over low-quality speakers', 'Low-end device play', 'Audio output processed cleanly without distortion', 'Medium', 'Minor', 'Voice Helper Usability'), ('Verify guidance instructions use simple language instead of technical terms', 'Groq prompt config', 'User completes complex form steps using basic instructions', 'High', 'Major', 'Form Guidance Clarity')]
}

def generate_cases():
    print("Generating User Acceptance Testing (UAT) cases...")
    ...
    output_filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), "uat_test_cases.xlsx")
