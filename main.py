...
async def upload_document(
    ...
    if audio_path != "":
        audio_url = f"http://172.29.184.191:8000/{audio_path.replace(chr(92), '/')}"

    return {
        "message": "Uploaded & processed successfully",
        "guidance": guidance_text,
        "audio_file": audio_url,
        "pdf_file": f"http://172.29.184.191:8000/{pdf_path.replace(chr(92), '/')}"
    }
...
def get_profile(email: str):
    ...
    if user.profile_image:
        profile_image_url = f"http://172.29.184.191:8000/{user.profile_image.replace(chr(92), '/')}"
...
async def update_profile(
    ...
    if image_path:
        profile_image_url = f"http://172.29.184.191:8000/{image_path.replace(chr(92), '/')}"
...
def get_history(email: str):
    ...
    for doc in docs:
        ...
        if doc.audio_path:
            audio_url = f"http://172.29.184.191:8000/{doc.audio_path.replace(chr(92), '/')}"
...
def get_form_details(doc_id: int):
    ...
    file_url = f"http://172.29.184.191:8000/uploads/{doc.file_name}"
